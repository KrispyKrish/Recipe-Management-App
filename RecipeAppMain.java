...
public class RecipeAppMain extends JFrame {
    ...
    private final RecipePanel recipePanel; // Initialize RecipePanel
    ...
    public RecipeAppMain() {
        ...
        recipePanel = new RecipePanel(); // Initialize the RecipePanel at the start
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            if (!name.isEmpty() && !ingredientsText.isEmpty() && !steps.isEmpty()) {
                ...
                for (String ingredientName : ingredientsText.split(",")) {
                    ingredients.add(new Ingredient(ingredientName.trim(), 0)); // Default calorie value
                }

                Recipe newRecipe = new Recipe(name, currentUser); // Constructor with author
                newRecipe.setSteps(steps);
                newRecipe.getIngredients().addAll(ingredients);

                globalRecipes.add(newRecipe);

                recipePanel.updateRecipeList(); // Update list in RecipePanel
    ...
    class RecipePanel extends JPanel {
        ...
        public RecipePanel() {
            ...
            commentButton.addActionListener(_ -> commentOnRecipe());
